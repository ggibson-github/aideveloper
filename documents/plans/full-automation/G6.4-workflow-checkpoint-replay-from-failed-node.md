<!-- Complete pass 1 2026-06-28 G6.4 -->

# G6.4: workflow checkpoint replay from failed node

**Parent:** [G6-index](G6-index.md) · **Branch G** · **Vision §19** · **Release:** v2.26

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

On node verify failure, active_workflow records failed_node_id and checkpoint artifacts; resume replays from that node after H2 fix—not full workflow restart unless graph changes.

See [Vision §9 — Branch G — Verification & quality plane](../../full-automation-vision-and-hierarchy.md#branch-g--verification--quality-plane) and [SEC-18-transistor-model-a-to-z](SEC-18-transistor-model-a-to-z.md).

## Purpose

G6.4 defines workflow checkpoint replay from failed node for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `G6.4` only; siblings under `G6` must not duplicate this spec.
...
1. state.pursuit.active_workflow.failed_node_id set; completed_nodes preserved.
2. Retry increments node retry_count; gate edges handle retry vs H2 per B6.4.
3. Graph edit invalidates downstream completed nodes via E5.4 stale mark.
4. preCompact snapshot H6 includes active_workflow for autopilot resume.
5. Manual operator reset requires journal entry clearing failed state.
...
{
  "node": "G6.4",
  "description": "workflow checkpoint replay from failed node",
...
1. Map `G6.4` to release row in [SEC-15-index](SEC-15-index.md) (v2.26).
...
5. Link from [G6-index](G6-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids G6.4` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids G6.4` passes
