<!-- Complete pass 1 2026-06-28 D1.7 -->

# D1.7: L6 transistor registered composable block

**Parent:** [D1-index](D1-index.md) · **Branch D** · **Vision §19** · **Release:** v2.24

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

L6 is the promotion ladder capstone: a reusable block registered in the transistor catalog with typed I/O, executor binding, and verify—composable in generator workflows. L2 scripts often implement the executor inside an L6 manifest.
...
D1.7 defines l6 transistor registered composable block for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `D1.7` only; siblings under `D1` must not duplicate this spec.
...
1. Promotion target_level L6 adds docs/platform/transistors/<id>.json plus TRANSISTORS.md row.
2. L6 subsumes L2–L5 artifacts: a transistor may reference script path, skill wrapper, hook, or pack fragment.
3. Ephemeral L0 must not remain the steady state for any capability appearing in two workflows.
4. L6 definition of done extends D6 with transistor-specific criteria (D6.5).
5. Fork versus extend follows D5.2/D5.3 with semver on transistor_id.
...
{
  "node": "D1.7",
  "description": "l6 transistor registered composable block",
...
1. Map `D1.7` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
5. Link from [D1-index](D1-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids D1.7` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids D1.7` passes
