<!-- Complete pass 1 2026-06-28 SEC-15-v2.24 -->

# SEC-15-v2.24: Release v2.24 transistor schema registry list-transistors

**Parent:** [SEC-15-index](SEC-15-index.md) · **Branch SEC** · **Vision §19** · **Release:** v2.24

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Ships transistor.v1.json schema, docs/platform/transistors/ bootstrap set, list-transistors.py, TRANSISTORS.md generation, D1.7 L6 promotion path, E6 catalog integration.
...
SEC-15-v2.24 defines release v2.24 transistor schema registry list-transistors for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `SEC-15-v2.24` only; siblings under `SEC-15` must not duplicate this spec.
...
1. Bootstrap transistors: validate-workflow-run, verify-router-invoke, route-tier-preflight, dual-write-journal.
2. validate-workflow.py extended for transistor schema.
3. SEC-18 published as design authority.
4. Unit tests for schema and list-transistors.
5. No workflow-compose mandatory yet—registry foundation only.
...
{
  "node": "SEC-15-v2.24",
  "description": "release v2.24 transistor schema registry list-transistors",
...
1. Map `SEC-15-v2.24` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-15-v2.24` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-15-v2.24` passes
