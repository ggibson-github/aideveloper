<!-- Complete pass 1 2026-06-28 SEC-17-9 -->
...
**Resolved:** Machine-readable workflow JSON is required and authoritative for v2.25+; visual node editor is optional operator tooling in v2.28+, never a substitute for schema validation.
...
SEC-17-9 defines decision json dag authoritative visual editor optional for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `SEC-17-9` only; siblings under `SEC-17` must not duplicate this spec.
...
1. Evidence: headless SDK (I2), export-contract (J5) require JSON; UI-only blocks automation.
2. H1 plan may include mermaid for humans; validate-workflow-dag.py validates JSON only.
3. Game-dev-style blocks map 1:1 to JSON nodes—same registry.
...
{
  "node": "SEC-17-9",
  "description": "decision json dag authoritative visual editor optional",
...
1. Map `SEC-17-9` to release row in [SEC-15-index](SEC-15-index.md) (v2.25).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-9` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-9` passes
