<!-- Complete pass 1 2026-06-28 E7.4 -->
...
Extends E2.5: compose miss at workflow level enqueues L6 promotion with capability and I/O hints; product may proceed with explicit L0 node waiver in DAG metadata.
...
E7.4 defines workflow miss enqueue transistor promotion for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `E7.4` only; siblings under `E7` must not duplicate this spec.
...
1. Unresolved workflow nodes marked l0_waiver: true require journal rationale and expiry.
2. promotion_queue items link source_workflow_id and node_id for traceability.
3. Platform drain prioritizes misses blocking critical path nodes per D3.3.
4. Second workflow needing same miss without promotion triggers H2.
5. Divergence log records L0 waivers for G5.8 metrics.
...
{
  "node": "E7.4",
  "description": "workflow miss enqueue transistor promotion",
...
1. Map `E7.4` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.4` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.4` passes
