<!-- Complete pass 1 2026-06-28 B6.4 -->
...
Gate transistors are control-flow blocks: they consume verify results or S0 predicate output and select the next edge in the workflow DAG—pass, fail, retry, or H2—without LLM interpretation.
...
B6.4 defines gate transistor pass fail edge routing for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `B6.4` only; siblings under `B6` must not duplicate this spec.
...
1. Gate executor kinds: verify_result, file_exists, schema_match, budget_remaining, staleness_clear.
2. validate-workflow-dag.py requires explicit edges for every gate outcome; no implicit fall-through.
3. Fail edges default to structured H2 when retry_count exhausted; retry edges increment node-local counter in state.
4. Gate transistors rank as hard when fully S0; soft gates forbidden—control flow must be deterministic.
5. Miswired gate (missing edge) fails conformance at compose time, not at runtime surprise.
...
{
  "node": "B6.4",
  "description": "gate transistor pass fail edge routing",
...
1. Map `B6.4` to release row in [SEC-15-index](SEC-15-index.md) (v2.26).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids B6.4` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids B6.4` passes
