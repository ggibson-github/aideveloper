<!-- Complete pass 1 2026-06-28 F5.4 -->

# F5.4: shared transistors library template-packs _shared

**Parent:** [F5-index](F5-index.md) · **Branch F** · **Vision §19** · **Release:** v2.27

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

template-packs/_shared/transistors/ holds domain-agnostic blocks: git-commit, verify-pytest, route-tier-preflight, journal-dual-write, scaffold-module—imported by all industry packs.

See [Vision §8 — Branch F — Organization plane](../../full-automation-vision-and-hierarchy.md#branch-f--organization-plane) and [SEC-18-transistor-model-a-to-z](SEC-18-transistor-model-a-to-z.md).

## Purpose

F5.4 defines shared transistors library template-packs _shared for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `F5.4` only; siblings under `F5` must not duplicate this spec.
...
1. F5.1 micro-pack imports may add supplemental transistors without overriding _shared ids.
2. Shared transistor semver is organization-wide; breaking change requires migration notes.
3. list-transistors defaults to searching _shared then pack overlay.
4. Promotion from consumer repo targets _shared when capability is generic per D4.6 review.
5. CATALOG.md lists _shared transistors in dedicated section.
...
{
  "node": "F5.4",
  "description": "shared transistors library template-packs _shared",
...
1. Map `F5.4` to release row in [SEC-15-index](SEC-15-index.md) (v2.27).
...
5. Link from [F5-index](F5-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids F5.4` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids F5.4` passes
