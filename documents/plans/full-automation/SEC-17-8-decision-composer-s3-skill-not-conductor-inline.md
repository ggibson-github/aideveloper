<!-- Complete pass 1 2026-06-28 SEC-17-8 -->
...
**Resolved:** Workflow composition runs via dedicated workflow-composer skill (S3) with catalog-only reads; genius conductor approves and dual-writes, does not compose large DAGs inline.
...
SEC-17-8 defines decision composer s3 skill not conductor inline for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `SEC-17-8` only; siblings under `SEC-17` must not duplicate this spec.
...
1. Evidence: B3.1 genius thin turns, E4.3 allowed_reads cap—inline compose violates both.
2. Composer spawn follows orchestrate-subagents contract.
3. Economy tier forbidden for composer—graph mistakes are expensive.
...
{
  "node": "SEC-17-8",
  "description": "decision composer s3 skill not conductor inline",
...
1. Map `SEC-17-8` to release row in [SEC-15-index](SEC-15-index.md) (v2.26).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-8` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-8` passes
