<!-- Complete pass 1 2026-06-28 B6.3 -->
...
Default product turn executes exactly one workflow node—mirroring A2.2 one pipeline step. This prevents fuzzy multi-hop improvisation and keeps evidence per node auditable.
...
B6.3 defines one workflow node per product turn default for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `B6.3` only; siblings under `B6` must not duplicate this spec.
...
1. Preflight reads active_workflow.current_node_id; route-tier maps node to transistor executor class.
2. Hard transistors run via S0 or tool-operator; soft transistors spawn bounded S1 workers with output schema validation.
3. After node verify passes, advance current_node_id; dual-write completed_nodes[] and evidence paths.
4. Gate transistors may branch without consuming an extra product turn when S0 branch resolver handles pass/fail edges.
5. Batching multiple nodes per turn requires explicit pack policy or operator waiver in journal—never default.
...
{
  "node": "B6.3",
  "description": "one workflow node per product turn default",
...
1. Map `B6.3` to release row in [SEC-15-index](SEC-15-index.md) (v2.26).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids B6.3` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids B6.3` passes
