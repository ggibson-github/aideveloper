<!-- Complete pass 1 2026-06-28 D4.7 -->

# D4.7: platform work transistor extraction

**Parent:** [D4-index](D4-index.md) · **Branch D** · **Vision §19** · **Release:** v2.24

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Platform turn work type: distill repeated manual or L0 sequences into L6 transistor manifests—generalized I/O, script/tool executor, verify command, catalog registration.
...
D4.7 defines platform work transistor extraction for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `D4.7` only; siblings under `D4` must not duplicate this spec.
...
1. Input: promotion_queue item with traces from worker promotion_note or divergence log.
2. Output: docs/platform/transistors/<id>.json, unit test, TRANSISTORS.md row, staleness node.
3. Prefer hard executor extraction from existing L2 script before wrapping soft templates.
4. Run list-transistors.py --check-duplicates to satisfy G5.6 duplicate tooling control.
5. Regenerate CATALOG.md via D4.5 after extraction completes.
...
{
  "node": "D4.7",
  "description": "platform work transistor extraction",
...
1. Map `D4.7` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
5. Link from [D4-index](D4-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids D4.7` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids D4.7` passes
