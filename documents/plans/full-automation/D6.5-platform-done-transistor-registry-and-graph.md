<!-- Complete pass 1 2026-06-28 D6.5 -->

# D6.5: platform done transistor registry and graph

**Parent:** [D6-index](D6-index.md) · **Branch D** · **Vision §19** · **Release:** v2.24

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Platform promotion to L6 completes only when the transistor is registered, referenced by at least one workflow or task card, verify-tested, and wired in staleness graph (E5.4).
...
D6.5 defines platform done transistor registry and graph for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `D6.5` only; siblings under `D6` must not duplicate this spec.
...
1. D6.1–D6.4 remain necessary; D6.5 adds TRANSISTORS.md INDEX row and schema validation pass.
2. At least one docs/workflows/*.json or pack template references the new transistor_id.
3. tests/unit/test_transistor_<id>.py or transistor verify script exits 0.
4. docs/manifest/staleness.json registers node; reconcile-stale propagates bumps.
5. False complete audit: audit-hierarchy-depth equivalent for transistor registry coverage.
...
{
  "node": "D6.5",
  "description": "platform done transistor registry and graph",
...
1. Map `D6.5` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
5. Link from [D6-index](D6-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids D6.5` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids D6.5` passes
