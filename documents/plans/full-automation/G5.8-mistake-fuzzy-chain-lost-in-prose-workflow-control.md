<!-- Complete pass 1 2026-06-28 G5.8 -->

# G5.8: mistake fuzzy chain lost in prose workflow control

**Parent:** [G5-index](G5-index.md) · **Branch G** · **Vision §19** · **Release:** v2.26

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Fuzzy-chain mistake class: long improvised edit sequences without workflow node boundaries—neutralized by mandatory workflow-compose, one-node-per-turn, and transistor verify per step.
...
G5.8 defines mistake fuzzy chain lost in prose workflow control for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `G5.8` only; siblings under `G5` must not duplicate this spec.
...
1. Detect: implement turns without workflow_node_id when C6.1 applies; divergence log count spikes.
2. Control: block advance at preflight; require workflow bind or H2 waiver.
3. Metrics: SEC-15-v2.28 dashboard shows fuzzy-chain incidents per goal.
4. Related controls: G5.2 scope creep, G5.6 duplicate tooling, B4.3 compose-first.
5. Recovery: G6.4 checkpoint replay from last verified node.
...
{
  "node": "G5.8",
  "description": "mistake fuzzy chain lost in prose workflow control",
...
1. Map `G5.8` to release row in [SEC-15-index](SEC-15-index.md) (v2.26).
...
5. Link from [G5-index](G5-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids G5.8` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids G5.8` passes
