<!-- Complete pass 1 2026-06-28 E7.2 -->
...
Composer iterates template gaps: for each required capability, list-transistors, rank per E6.5, wire outputs to inputs, insert gate nodes at verify boundaries.
...
E7.2 defines stitch transistors into generator dag for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `E7.2` only; siblings under `E7` must not duplicate this spec.
...
1. Walk template or decomposed capability list in dependency order.
2. Each stitch adds node + edges; params from goal context and upstream output refs.
3. Subgraph reuse: reference pack standard_cells when F5.4 defines them.
4. Composer output includes unresolved[] array driving promotion_queue.
5. Human-readable mermaid summary optional in H1 plan; JSON is authoritative.
...
{
  "node": "E7.2",
  "description": "stitch transistors into generator dag",
...
1. Map `E7.2` to release row in [SEC-15-index](SEC-15-index.md) (v2.25).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.2` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.2` passes
