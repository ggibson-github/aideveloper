<!-- Complete pass 1 2026-06-28 E7.3 -->
...
validate-workflow-dag.py verifies DAG acyclicity, typed edge compatibility, gate edge completeness, and transistor preconditions satisfiable from goal inputs—before implement starts.
...
E7.3 defines validate wiring preconditions postconditions s0 for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `E7.3` only; siblings under `E7` must not duplicate this spec.
...
1. Run at workflow-compose phase exit and on any DAG edit before resume.
2. Type mismatch between node output and downstream input fails closed with line-level report.
3. Unknown transistor_id or stale version fails per E5.4 graph.
4. Integrates with validate-workflow.py in CI on docs/workflows/ changes.
5. Pass result stored in state.pursuit.active_workflow.validation_hash.
...
{
  "node": "E7.3",
  "description": "validate wiring preconditions postconditions s0",
...
1. Map `E7.3` to release row in [SEC-15-index](SEC-15-index.md) (v2.25).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.3` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids E7.3` passes
