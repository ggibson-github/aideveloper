<!-- Complete pass 1 2026-06-28 SEC-17-7 -->
...
**Resolved:** One transistor = one verify boundary. If verify cannot be expressed as a single command or predicate, split the block. Maximum soft scope = one bounded S1 job with schema output.
...
SEC-17-7 defines decision transistor granularity one verify boundary for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `SEC-17-7` only; siblings under `SEC-17` must not duplicate this spec.
...
1. Decision recorded 2026-06-28; supersedes informal larger-block proposals.
2. Evidence: G5.4 skipped tests, G5.1 hallucinated done—per-node verify contains blast radius.
3. Implement feature whole-task is a workflow, not one transistor.
4. SEC-18 §Granularity references this decision.
...
{
  "node": "SEC-17-7",
  "description": "decision transistor granularity one verify boundary",
...
1. Map `SEC-17-7` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-7` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids SEC-17-7` passes
