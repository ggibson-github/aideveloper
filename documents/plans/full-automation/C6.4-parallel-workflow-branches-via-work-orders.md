<!-- Complete pass 1 2026-06-28 C6.4 -->

# C6.4: parallel workflow branches via work orders

**Parent:** [C6-index](C6-index.md) · **Branch C** · **Vision §19** · **Release:** v2.27

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Independent workflow branches—nodes with no path between them—may run in parallel via C3.2 work orders and C4.1 lane leases, each branch carrying its own current_node pointer under active_workflow.
...
C6.4 defines parallel workflow branches via work orders for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `C6.4` only; siblings under `C6` must not duplicate this spec.
...
1. validate-workflow-dag.py identifies parallel-ready antichains; orchestrate-program assigns lanes per branch.
2. state.pursuit.active_workflow.branches[] holds per-branch current_node_id and lease owner.
3. Join nodes declare required upstream branch outputs as typed inputs—manifest integration contract.
4. Parallel branches must not share mutable output paths without explicit gate merge transistor.
5. Program goal_verify waits for all branches to reach terminal nodes before G2 rollup.
...
{
  "node": "C6.4",
  "description": "parallel workflow branches via work orders",
...
1. Map `C6.4` to release row in [SEC-15-index](SEC-15-index.md) (v2.27).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids C6.4` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids C6.4` passes
