<!-- Complete pass 1 2026-06-28 E6.5 -->
...
Extends E2.3 ranking for workflow node binding: hard_transistor > script > soft_transistor > playbook > skill > facts. Gate transistors are not capability substitutes—they are edges.
...
E6.5 defines compose rank hard transistor script soft for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `E6.5` only; siblings under `E6` must not duplicate this spec.
...
1. When filling a workflow node, composer applies E6.5 before E2.3 legacy single-component compose.
2. Ties break toward higher maturity and pack-authored transistors (F1.9).
3. Invoking soft when hard exists requires B4.4 divergence with justification.
4. Facts disambiguate only; never replace executable transistors in ranked slots.
5. Conformance check in validate-workflow.py when task card transistor_id mismatches rank.
...
{
  "node": "E6.5",
  "description": "compose rank hard transistor script soft",
...
1. Map `E6.5` to release row in [SEC-15-index](SEC-15-index.md) (v2.24).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids E6.5` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids E6.5` passes
