<!-- Complete pass 1 2026-06-28 F1.9 -->

# F1.9: pack transistors and generator workflows

**Parent:** [F1-index](F1-index.md) · **Branch F** · **Vision §19** · **Release:** v2.27

## Reader narrative
<!-- prose-source: agent transistor-expansion 2026-06-28 -->

Pack schema adds workflows/*.json generator templates and transistors/*.json domain blocks—game studio mesh/rig/import, data platform ingest/model/deploy, software delivery scaffold chains.
...
F1.9 defines pack transistors and generator workflows for the agent-driven expert system. Transistor & generator workflow model (§19).
## Scope

- Owns `F1.9` only; siblings under `F1` must not duplicate this spec.
...
1. company.yaml references default_workflow_template per pipeline_id.
2. Roles bind to subgraphs of pack workflows via F1.2 pipeline slice fields.
3. Pack CI validates all transistors and workflows against v1 schemas.
4. Reference implementations in SEC-15-v2.20 and v2.21 include transistor libraries.
5. Export upward from consumer repos via D4.6 pack fragment export.
...
{
  "node": "F1.9",
  "description": "pack transistors and generator workflows",
...
1. Map `F1.9` to release row in [SEC-15-index](SEC-15-index.md) (v2.27).
...
5. Link from [F1-index](F1-index.md).
...
| Check | Command |
|-------|---------|
| Completeness | `python scripts/automation/audit-hierarchy-depth.py --strict --ids F1.9` |
...
- [ ] `python scripts/automation/audit-hierarchy-depth.py --strict --ids F1.9` passes
